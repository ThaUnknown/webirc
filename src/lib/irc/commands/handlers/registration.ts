...
import { intersection, difference, each, uniq } from '../../util'
import { text2arr, arr2base } from 'uint8-util'
...
const handlers = {
  ...
  CAP: function (command, handler) {
    ...
    want = uniq(want.concat(handler.request_extra_caps))

    switch (command.params[1]) {
      case 'LS':
        // Compute which of the available capabilities we want and request them
        request_caps = intersection(capabilities, want)
      ...
      case 'ACK':
        if (capabilities.length > 0) {
          ...
          handler.network.cap.enabled = uniq(handler.network.cap.enabled.concat(capabilities))

          // Update list of capabilities we would like to have but that aren't enabled
          handler.network.cap.requested = difference(
      ...
      case 'DEL':
        // Update list of enabled capabilities
        handler.network.cap.enabled = difference(
  ...
  AUTHENTICATE: function (command, handler) {
    ...
    const auth_str = saslAuth.account + '\0' +
            saslAuth.account + '\0' +
            saslAuth.password
    const b64 = arr2base(text2arr(auth_str, 'utf8'))
