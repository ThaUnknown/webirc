...
export default class IrcCommandHandler extends EventEmitter {
  ...
  executeCommand (irc_command) {
    ...
    if (ircNumerics[irc_command.command.toUpperCase()]) {
      command_name = ircNumerics[irc_command.command.toUpperCase()]
    }
  ...
  requestExtraCaps (cap) {
    this.request_extra_caps = uniq(this.request_extra_caps.concat(cap))
  }
 ...
 */
  parseModeList (mode_string, mode_params) {
    ...
    const hasParam = function (mode, isAdd) {
      ...
      if (find(always_param, matchMode)) {
        return true
      }

      if (isAdd && find((chanmodes[2] || '').split(''), matchMode)) {
        return true
      }
